...
nums가 매개변수로 주어질 때, N/2마리의 폰켓몬을 선택하는 방법 중, 
...
배열의 길이의 n/2만큼 가져갈수 있는데
여기서 내가 가져갈 수 있는 종류가 총 몇개인지

[3,1,2,3]	2 
[3,3,3,2,2,4]	3 
[3,3,3,2,2,2]	2 
1만 숫자도 1~10000 5000개만 가져갈수있는데 유니크한 숫자가 1만이면 결국에 선택할수 총 종류는 5천

결론 : 유니크한 숫자의 갯수를 구하면돼, 근데 이게 배열의 길이보다 크면 (배열의 길이/2)를 리턴, 그거보다 작으면 유니크한 숫자의 갯수 리턴
...
function solution(array) {
  const pokemonBox = array;

  const numberOfCanGet = pokemonBox.length / 2;

  const uniquePokemonBox = new Set(pokemonBox);

  const numberOfUniquePokemon = uniquePokemonBox.size;

  if (numberOfCanGet > numberOfUniquePokemon) return numberOfUniquePokemon;
  else return numberOfCanGet;
}

console.log(solution([5, 5, 5, 5, 5, 5]));

/*
테스트 1 〉	통과 (0.04ms, 29.7MB)
...
테스트 14 〉	통과 (0.07ms, 29.7MB)
테스트 15 〉	통과 (0.06ms, 30MB)
테스트 16 〉	통과 (0.53ms, 29.8MB)
테스트 17 〉	통과 (0.49ms, 30.2MB)
테스트 18 〉	통과 (0.38ms, 29.7MB)
테스트 19 〉	통과 (0.29ms, 29.8MB)
테스트 20 〉	통과 (0.24ms, 29.9MB)
*/
